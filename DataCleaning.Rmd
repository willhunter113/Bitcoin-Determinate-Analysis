...
CPI <- cpiAuc[1:756,]
FedFunds<- fedFunds[1:756,]
Treasury3 <- treasuryYield3[1:756,]
Treasury10 <- treasuryYield10[1:756,]
Treasury30 <- treasuryYield30[1:756,]
Civillian<- unemployCivilian[1:756,]

Fed <- cbind(Fed,saveRate$Value,disposeIncome$Value,CPI$Value,FedFunds$Value,Treasury3$Value,Treasury10$Value, Treasury30$Value, Civillian$Value)

FederalReserve <- Fed[1:536,]
```

Renaming columns in the aggregated Federal Reserve Data
```{r}
names(FederalReserve)[names(FederalReserve)== "disposeIncome.Date"] <- "Date"
...
```{r}
IndependentVars <- merge(FederalReserve,UMich, by = "Date")
```

Adding Bitcoin and Stock Index Data to independent Variables
```{r}
Data <- left_join(aDependentVars,IndependentVars, join_by = "Date")
```
